<?php
include("../inc/header.php");
?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/css/castracao.css">
    <link rel="stylesheet" href="../css/css/responsividade/telacastracao.css">
    <link rel="stylesheet" href="../css/css/whatsapp.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.css">
    <title>Castração de Felinos</title>

    <style>
        .modalLog {

            position: absolute;
            /* Modal posicionado de forma absoluta em relação ao body */
            top: 50%;
            /* Posiciona o modal verticalmente no centro */
            left: 25%;
            /* Posiciona o modal horizontalmente no centro */
            /* Garante que o modal fique acima de outros elementos */
        }
    </style>
</head>
<style>
@font-face {
    font-family: 'poppins';
    src: url("/VacinePet/css/font/poppins/Poppins-Regular.ttf");
}

h2 {
    font-weight: 400;
    font-family: baloo;
}

p {
    font-family: poppins;
}

.btn-custom {
    border: none;
}
</style>


<body>
    <div class="container">
        <h1 id="title">CASTRAÇÃO DE FELINOS</h1>
        <div class="text">
            <h2 id="sub-title-cas" class="sub">O que é Castração?</h2>
            <p id="p1">A castração é um procedimento cirúrgico que remove os órgãos reprodutivos dos felinos. Para os
                machos,
                são removidos os testículos,
                e para as fêmeas,
                os ovários e o útero. Este procedimento oferece diversos benefícios,
                como a prevenção de doenças,
                a redução de comportamentos indesejados e o controle da população de animais.<br></p>
            <h2 id="sub-title-esp" class="sub1">Especialização e Valor</h2>
            <p id="p2">Nossa veterinária é especializada exclusivamente na castração de felinos. Não realizamos
                castração de cães. Para mais informações e consulta de valores,
                entre em contato conosco. Estamos prontos para oferecer o melhor cuidado para seu animal de estimação.
            </p>
        </div><a
            href="https://wa.me/5512991445347?text=Olá!%20Tenho%20interesse%20pelo%20serviço%20de%20castração.%20Poderia%20me%20ajudar%20com%20mais%20informações?"
            target="_blank" title="Fale conosco no WhatsApp"><button id="btn"
                class="btn btn-primary btn-custom">Consulte agora</button></a>
    </div>
    <div class="container-img"></div>
</body><?php include("../inc/footer.php") ?>

</html>